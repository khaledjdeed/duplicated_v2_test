...
import { mockAuditLogs, MockAuditLog } from '../data/mockData';
...
export function AuditLogsView() {
  ...
  // Use mock audit logs data
  const auditLogs: MockAuditLog[] = mockAuditLogs;
  ...
  const handleExport = () => {
    ...
    const exportLogEntry: MockAuditLog = {
      id: `log-${Date.now()}`,
      user_id: user?.id || '',
